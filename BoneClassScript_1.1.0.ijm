...
function CreateResultsReplica(TableName, TableW, TableH, TableLocX, TableLocYStart){
	...
	Table.create(TableName); // Create a table
	for (i = 0; i < ResultsHeading.length; i++) {
		Table.setColumn(ResultsHeading[i]);
	}
	Table.setLocationAndSize(TableLocX, TableLocYStart, TableW, TableH, TableName);
...
function DataGrabberFunction(InputTable, OutputTableName){
	
	selectWindow(OutputTableName);
	nTable = Table.size;
	ResultsHeading = split(String.getResultsHeadings);
	// loop over result headings
	for (i = 0; i < ResultsHeading.length; i++) {
		//print(ResultsHeading[i]);
		// loop over results
		for (z = 0; z < nResults; z++) {
			// get result
			Result = getResultString(ResultsHeading[i], z);
			//print("Result: " + Result);
			row = nTable + z;
			Table.set(ResultsHeading[i], row, Result);
		}	
	}
	Table.update;
}
...
for (j = 0; j < NewFileList2.length; j++) {
	...
	// load image 
	run("Bio-Formats Importer", "open=" + InputPath + " autoscale color_mode=Default open_files rois_import=[ROI manager] view=Hyperstack stack_order=XYCZT");
	...
	Log = Array.concat(Log, OriginalStack);
	...
	if (isOpen("ResultsCollection.csv") == false){
		...
		//Create Collection table
	 	CreateResultsReplica("ResultsCollection.csv", TableW, TableH, TableLocX, 0);
	
		// Set position
	}
    // Retrieve data from the Results table and save to Collection table
    DataGrabberFunction("Results", "ResultsCollection.csv");
	...
	if (ContinueOrStopMacro == 1) {
		...
		selectWindow("ResultsCollection.csv");
		Table.save(outDir + "ResultsCollection.csv");
		run("Close");
